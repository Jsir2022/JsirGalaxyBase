...
## 8. 当前权限与运维收口（2026-08-11）

- 个人 Base Vault 固定 27 格，只允许账户本人或管理员执行访问；企业与公共 Base Vault 固定 54 格。
- 企业/公共仓目前只有容量、排序和权限接口地基。权威成员/岗位适配器未接入时必须默认拒绝，不能因为账户存在就开放 UI 或存取。
- 权限按 `VIEW`、`DEPOSIT`、`WITHDRAW`、`SORT` 分离。未来企业岗位和公共决议只能实现对应授权端口，不得绕过 Base Vault 服务直接修改槽位。
- 每次部署或故障恢复后运行 `scripts/market-audit.sh --strict`。除了市场、银行和交付异常，该命令还检查：
  - `warehouse_operation_log` 中的 `CREATED`、`PROCESSING`、`RECOVERY_REQUIRED`；
  - 个人 27 格、企业/公共 54 格容量契约；
  - `warehouse_slot.slot_index` 是否落在账户容量范围内；
  - 待处理仓储操作是否有对应的槽位变更审计记录。
- `--strict` 非零退出意味着需要人工查看输出记录，不能靠重试、补发或直接改余额表消除异常。
