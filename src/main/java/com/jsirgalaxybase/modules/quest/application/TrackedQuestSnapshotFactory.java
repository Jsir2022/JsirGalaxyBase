...
public final class TrackedQuestSnapshotFactory {
    ...
    public TerminalTrackedQuestSnapshot create(ParticipantId participant,long generatedAt){List<TerminalTrackedQuestSnapshot.Quest> quests=new ArrayList<TerminalTrackedQuestSnapshot.Quest>();for(QuestCenterEntry entry:query.load(participant).getQuests()){if(!entry.isTracked())continue;quests.add(quest(entry));if(quests.size()==5)break;}return new TerminalTrackedQuestSnapshot(quests,generatedAt);}
