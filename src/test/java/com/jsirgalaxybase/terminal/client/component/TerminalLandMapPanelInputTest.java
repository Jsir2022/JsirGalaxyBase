...
public class TerminalLandMapPanelInputTest {
    ...
    public void clickDragRightClickAndEscapeRemainDistinct() {
        ...
        panel.setBounds(new GuiRect(10, 20, 450, 270));

        assertTrue(panel.mouseClicked(null, 235, 155, 0));
        assertTrue(panel.mouseReleased(null, 275, 155, 0));
        assertEquals(0, handler.selects);
        assertEquals(1, handler.viewports);

        assertTrue(panel.mouseClicked(null, 235, 155, 1));
        assertTrue(panel.isContextOpen());
        assertTrue(panel.keyTyped(null, '\0', Keyboard.KEY_ESCAPE));
    ...
    public void contextActionAndKeyboardShortcutsUseExistingHandlers() {
        ...
        panel.mouseClicked(null, 235, 155, 1);
        assertTrue(panel.mouseClicked(null, 245, 179, 0));
        assertEquals(1, handler.contextActions);
        assertFalse(handler.lastUnclaim);

        assertTrue(panel.keyTyped(null, 'f', Keyboard.KEY_F));
        assertTrue(panel.keyTyped(null, '+', Keyboard.KEY_ADD));
        assertTrue(panel.keyTyped(null, '-', Keyboard.KEY_MINUS));
