...
public final class QuestManagementVisualDocument extends ComponentUiDocument {
    ...
    private UiElement behaviorEditor(){final QuestManagementVisualModel.Definition summary=model.getDetail().getSummary();UiElement.Builder card=UiElement.type("Card").key("quest-behavior-picker").child(label("quest-behavior-title","重复与行为选项 · "+(edit.behaviorPage+1)+" / 3",true));
        ...
        else{card.child(optionToggle("quest-behavior-global","全局任务","behavior.global"));card.child(optionToggle("quest-behavior-share","共享全局进度","behavior.globalShare"));card.child(optionField("quest-behavior-icon","图标资源","behavior.icon"));card.child(optionField("quest-behavior-update-sound","进度音效","behavior.updateSound"));card.child(optionField("quest-behavior-complete-sound","完成音效","behavior.completeSound"));}
        card.child(UiElement.type("Row").key("quest-behavior-pager").child(button("quest-behavior-prev","‹",new Runnable(){public void run(){edit.behaviorPage=Math.max(0,edit.behaviorPage-1);invalidation.run();}},edit.behaviorPage>0)).child(button("quest-behavior-next","›",new Runnable(){public void run(){edit.behaviorPage=Math.min(2,edit.behaviorPage+1);invalidation.run();}},edit.behaviorPage<2)).build());
